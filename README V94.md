PinballPoolGameV94 changes

New coin door settings
Left ball kicker can be set on at ball start (never, for ball 1, or for all balls).  Since kicker lamp is shared with the spinner, there is a brief flashing sequence indicating status at the ball start.
Simplified Chimes - In most cases where double chime hits were used, a single chime is used.
Silent Match - Match mode can now have chiming turned off.

Other Changes

Goals status report displays any time the ball is cradled on the flipper for a few seconds in addition to at the beginning of the ball.
Revised lighting and animation routines.  
Revised display handling for game features.  Scoring from a game feature is briefly held on the non-player displays.
Sound effects adjusted to make mode starts and ends more clear.  Some sound effect updates.
